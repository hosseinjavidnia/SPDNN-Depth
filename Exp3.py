...
def load_dataset_Train_Validation():
    TrainValidationData = h5py.File('TrainValidateBeforeSeg.mat')

    TrainX = TrainValidationData['TrainX']
    TrainY = TrainValidationData['TrainYBefore']
    ValidationX = TrainValidationData['ValidateX']
    ValidationY = TrainValidationData['ValidateYBefore']
...
networki = lasagne.layers.InputLayer(shape=(None , 2 , 80 , 264))
...
for epoch in range(num_epochs):
    ...
    if validationError < validationErrorBest:
            ...
            with open('netBestAttempt5SegBefore1.pickle' , 'wb') as handle:
    ...
    scipy.io.savemat('LossesAttempt5SegBefore1.mat', mdict={'TrainLoss': TRAINLOSS, 'ValidationLoss': VALIDATIONLOSS})
